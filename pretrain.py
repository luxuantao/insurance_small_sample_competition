...
wandb.init(
        project="small_samples",
        name=arg_dict['run_name'],
    )
...
# 注意修改要格式化的数据源和存储文件
if arg_dict["format_data"]:
    all_data = load_json(os.path.join(arg_dict["data_dir"], "train_data.json"))
    all_data = instruction_format(all_data)
    EVAL_NUM = int(0.2 * len(all_data))  # TODO
    random.shuffle(all_data)
    write2json(all_data[:-EVAL_NUM], os.path.join(arg_dict["data_dir"], "{}_train.json".format(dataset_name)), "train data")
    write2json(all_data[-EVAL_NUM:], os.path.join(arg_dict["data_dir"], "{}_dev.json".format(dataset_name)), "dev data")
...
dev_data_raw = load_json(data_files["validation"])
...
def get_metrics_results(eval_preds):
    ...
    def reconstruct_predictions(eval_preds):
        ...
        eval_res = []
        for raw_data, pred in zip(dev_data_raw, decoded_preds):  # TODO check
...
class TestCallback(TrainerCallback):
    def on_evaluate(self, args, state, control, **kwargs):
        last_metric = state.log_history[-1]["eval_macro_f1"]
